...
const AddAssignment = ({ onClick, func }) => {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    const formattedDueDate = new Date(`${dueDate}T${dueTime}`).toLocaleString('en-GB', {
      day: '2-digit',
      month: '2-digit',
      year: 'numeric',
      hour: '2-digit',
    ...
    fetch("http://localhost:3000/createAssignment", {
      ...
      body: JSON.stringify({
        ...
        assignmentDetails : {name: assignmentName,
          due: formattedDueDate, problemStatement: "problemStatement", answerpdf : "answer"},
      }),
    }).then((data) => {
      data.json().then((data) => {
  ...
  return (
    <div className="relative h-[77%] overflow-y-scroll">
      <button className="absolute top-0 left-0 m-4" onClick={(e) => onClick(e, func)}>
        Back
      </button>
      <div className="max-w-md mx-auto mt-8 p-6 bg-white rounded-md shadow-md">
        <h2 className="text-3xl font-bold mb-4 text-gray-800">Create Assignment</h2>
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label htmlFor="assignmentName" className="block text-sm font-semibold text-gray-600">
              Assignment Name:
            </label>
            <input
              type="text"
          ...
          <div>
            <label htmlFor="dueDate" className="block text-sm font-semibold text-gray-600">
              Due Date:
            </label>
            <input
              type="date"
          ...
          <div>
            <label htmlFor="dueTime" className="block text-sm font-semibold text-gray-600">
              Due Time:
            </label>
            <input
              type="time"
          ...
          <div>
            <label htmlFor="assignmentFile" className="block text-sm font-semibold text-gray-600">
              Assignment File (PDF):
            </label>
            <input
              type="file"
          ...
          <div>
            <label htmlFor="answerFile" className="block text-sm font-semibold text-gray-600">
              Answer File (PDF):
            </label>
            <input
              type="file"
