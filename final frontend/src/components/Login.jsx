...
function Login() {
  ...
  const handleLogin = async (event) => {
    ...
    if (data.token) {
      ...
      console.log('user_id', data.token.user._id)
      ...
      if (data.token.user.role === 'teacher') navigate('/');
      else if (data.token.role === 'student') navigate('/student');
    } else {
      setVerified("false")
    }
          ...
          <div className='or'>
            or
              ...
              <form onSubmit={handleLogin}>
                <input className='input-feild' name='email' type="email" placeholder='Enter your email' onChange={(e) => {
                  setEmail(e.target.value)
                }}/>
                {
                    !forgotPass &&
                    <div className='password-section'>
                      <input className='input-feild' name='password' type={showPassword ? 'text' : 'password'}
                ...
                {
                    forgotPass &&
                    <button className='forgot-button' onClick={() => {
                      handleForgotPass()
                    }}>
                      send reset mail
                    </button>
